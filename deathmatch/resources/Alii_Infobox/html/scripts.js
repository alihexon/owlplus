    ...
    window.addEventListener('message', function (event) {
        if (event.data.action == 'open') {
            ...
            if (event.data.type == 'success') {
                ...
                sound.play();
			} else if (event.data.type == 'work') {
                $(`.title-${number}`).html(event.data.title).css({
                    "font-size": "16px",
                    "font-weight": "600"
                })
                $(`.notification_main-${number}`).addClass('work-icon')
                $(`.wrapper-${number}`).addClass('work work-border')
                sound.play();
                ...
                $(`.notification_main-${number}`).addClass('lock-icon')
                $(`.wrapper-${number}`).addClass('lock lock-border')
                sound.play();
                ...
                $(`.notification_main-${number}`).addClass('money-icon')
                $(`.wrapper-${number}`).addClass('money money-border')
                sound.play();
